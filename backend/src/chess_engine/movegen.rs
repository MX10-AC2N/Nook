...
use crate::chess_engine::types::{Bitboard, Color, Move, MoveFlags, PieceType, Square};
...
fn generate_pawn_moves(pos: &Position, us: Color, moves: &mut Vec<Move>) {
    ...
    for from in pawns.iter() {
        ...
        if let Some(ep_sq) = pos.en_passant {
            if t.pawn_attacks(us, from).is_set(ep_sq) {
...
use crate::chess_engine::types::CastlingRights;
