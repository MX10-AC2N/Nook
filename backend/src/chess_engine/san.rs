...
use crate::chess_engine::movegen;
use crate::chess_engine::types::{Move, PieceType, Square};
...
pub fn parse_san(pos: &Position, san: &str) -> Result<Move, crate::chess_engine::types::ChessError> {
    ...
    if chars.is_empty() {
        return Err(crate::chess_engine::types::ChessError::InvalidMove {
            from: String::new(),
            to: String::new(),
    ...
    let (chars, promotion) = if chars.len() >= 2 && chars[chars.len() - 2] == '=' {
        ...
        let promo = match promo_char {
            ...
            _ => {
                return Err(crate::chess_engine::types::ChessError::InvalidPromotion(
    ...
    let dest = Square::from_algebraic(&dest_str)
        .ok_or_else(|| crate::chess_engine::types::ChessError::InvalidSquare(dest_str.clone()))?;
    ...
    match candidates.len() {
        0 => Err(crate::chess_engine::types::ChessError::InvalidMove {
            from: String::new(),
            to: san.to_string(),
            reason: format!("no legal move matches SAN '{san}'"),
        }),
        1 => Ok(*candidates[0]),
        _ => Err(crate::chess_engine::types::ChessError::InvalidMove {
            from: String::new(),
            to: san.to_string(),
...
fn find_castling(
    ...
    legal
        ...
        .copied()
        .ok_or_else(|| crate::chess_engine::types::ChessError::InvalidMove {
...
mod tests {
    ...
    use crate::chess_engine::board::Position;
    use crate::chess_engine::movegen;
    use crate::chess_engine::types::MoveFlags;
