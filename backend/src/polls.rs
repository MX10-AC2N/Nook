    ...
    match row {
        ...
        Some(row) => {
            ...
            if row.closed_at.is_some() {
                return (
                    StatusCode::BAD_REQUEST,
                    Json(json!({ "message": "Sondage déjà fermé" })),
                )
                    .into_response();
            }
            // Fermer le sondage
            let now = Utc::now().timestamp();
            if let Err(e) = sqlx::query("UPDATE polls SET closed_at = ? WHERE id = ?")
                .bind(now)
                .bind(&poll_id)
                .execute(&state.db)
                .await
            {
                tracing::error!(error = %e, "close_poll UPDATE polls");
                return (
                    StatusCode::INTERNAL_SERVER_ERROR,
                    Json(json!({ "message": "Erreur fermeture sondage" })),
                )
                    .into_response();
            }
