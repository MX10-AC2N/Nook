...
name: nook-fin
category: "devops"
description: "Session exit command — clean shutdown with full context persistence using new .hermes/ structure. Run at end of any session to summarize work, update hermes/ docs, push state, and exit cleanly."
---
...
### 3. Mettre à jour .hermes/hermes/ (MON ESPACE PERSO)

**C'EST CRITIQUE** — Utiliser les nouveaux fichiers dans `hermes/`:

| Fichier | Action | Contenu |
|---------|--------|---------|
| `.hermes/hermes/active-session.md` | ✅ Mettre à jour | Session en cours, ce qui vient d'être fait |
| `.hermes/hermes/known-issues.md` | ✅ Ajouter/Mettre à jour | Nouveaux bugs, pièges évités, leçons |
| `.hermes/hermes/hermes-memory.md` | ✅ Mettre à jour | Infos critiques (tokens, comptes, règles) |
| `.hermes/hermes/preferences.md` | ✅ Vérifier | Préférences utilisateur inchangées |

**Procedure :**
```python
# a) Lire l'état actuel
# read_file(path='.hermes/hermes/active-session.md')

# b) Mettre à jour avec le résumé de session
# write_file(path='.hermes/hermes/active-session.md', content='...')

# c) Ajouter les nouveaux bugs/pièges dans known-issues.md
# read_file(path='.hermes/hermes/known-issues.md')
# puis ajouter les nouveaux éléments

# d) S'assurer que hermes-memory.md contient les infos critiques
# read_file(path='.hermes/hermes/hermes-memory.md')
```

### 4. Mettre à jour .hermes/project/ (REFERENCE PROJET)

| Fichier | Action | Contenu |
|---------|--------|---------|
| `.hermes/project/project-state.md` | ✅ Mettre à jour | Version, branche, CI status, derniers commits |
| `.hermes/project/BUGS.md` | ✅ Marquer bugs corrigés | Bugs actifs, résolus |

### 5. Archiver si nécessaire
```bash
# Si SESSIONS.md existe encore quelque part, s'assurer qu'il est dans archive/sessions/
if [ -f .hermes/SESSIONS.md ]; then
  mv .hermes/SESSIONS.md .hermes/archive/sessions/
fi
```
...
**Checklist skills :**
- [ ] Lister tous les skills existants du projet (`skills/` dans `.hermes/`)
...
Vérifier que chaque fichier `.hermes/skills/*/SKILL.md` modifié est bien poussé.
...
✅ Résumé: .hermes/hermes/active-session.md
✅ Bugs: .hermes/hermes/known-issues.md
✅ Contexte: .hermes/hermes/hermes-memory.md
✅ Projet: .hermes/project/project-state.md
...
Prochaine session: Je lirai .hermes/hermes/ AU DEMARRAGE (hook session-start-tools.md)
...
- **TOUJOURS** utiliser la nouvelle structure `.hermes/hermes/` pour l'espace perso
