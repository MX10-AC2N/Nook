...
### BUG-09 : Frontend build échoue (Svelte syntaxe)
- **Status :** 🔴 EN COURS
- **Symptôme :** `npm run build` échoue avec des erreurs de syntaxe Svelte (backslashes invalides, apostrophes non échappées)
- **Cause :** Caractères invalides dans `frontend/src/routes/chat/+page.svelte` (backslashes, apostrophes dans les strings)
- **Actions :** 
  - ✅ Apostrophes échappées dans les strings (l.205, l.823)
  - ✅ Backslashes supprimés du button tag
  - ❌ Build échoue encore (erreur Svelte compilation)
- **CI :** https://github.com/MX10-AC2N/Nook/actions/runs/25247954004
- **Prochaines étapes :** Vérifier le log CI complet pour identifier l'erreur restante
...
### CI-003 : Node.js 20 deprecated (Frontend.yml)
- **Statut :** ✅ FIXÉ (commit 60e11299)
- **Action :** Migré vers Node.js 24
- **Détails :** `actions/checkout@v4`, `actions/setup-node@v4` forcés en Node.js 24
...
8. ✅ Pas de backslashes dans les fichiers Svelte (syntaxe invalide)
9. ✅ Apostrophes dans les strings JS échappées (`\'` ou guillemets doubles)