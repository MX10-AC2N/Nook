...
### [APP-E2E-07] waitForResponse() race condition avec goto() — Session 32

`waitForResponse()` ne capture que les réponses arrivant APRÈS son enregistrement.
Si le composant déclenche une requête dans `onMount()` et que le test appelle
`waitForResponse()` après `waitForAppReady()` (qui attend la fin du loading), la
réponse est déjà passée → timeout 10s garanti.

**Solutions :**
1. Enregistrer `waitForResponse()` AVANT le `goto()` : `Promise.all([waitForResponse(...), page.goto(...)])`
2. Ou remplacer par `expect(page.locator('.element')).toBeVisible({timeout: 15_000})`
   → attend directement le résultat DOM, indépendant du timing réseau

→ Règle : pour les données chargées dans `onMount`, toujours utiliser la stratégie DOM.

### [APP-E2E-08] Rate limit global pollue les tests parallèles — Session 32

Le rate limiter Nook est **global** (pas par IP, pas par user) : `Quota::per_minute(10)`.
Avec 27 workers CI en parallèle, un test qui flood `/api/auth/login` × 15
épuise le quota pour TOUS les workers → les `loginAsAdmin` concurrents reçoivent 429.

**Fix :**
1. `test.describe.serial('Rate Limiting')` → sérialise le groupe, attend la fin avant de libérer les workers
2. Ajouter un retry 429 avec délai dans `loginAsAdmin` (défense en profondeur)

→ Règle : tout test qui épuise intentionnellement un quota global doit être `serial`.

### [APP-E2E-09] loginAs flaky sous charge CI — Session 32

Avec 27 workers simultanés, `unlockCrypto()` (génération clés E2EE + POST `/api/auth/public-key`)
peut ralentir le `goto('/chat')` post-login. Le click sur "Se connecter" réussit mais
la navigation dépasse 15s → `toHaveURL` timeout.

**Fix :** Retry automatique dans `loginAs` — si `toHaveURL` timeout, refill + re-submit + wait.
Attention : le retry ne masque pas un bug de login, il compense la latence CI.

### [APP-E2E-10] CreateGameRequest.opponent ≠ ai_difficulty — Session 32

Le backend `chess.rs` attend `{ opponent: "easy"|"medium"|"hard"|"human" }` dans `CreateGameRequest`.
Le frontend et les tests envoyaient `{ ai_difficulty: "medium" }` — champ ignoré silencieusement.
Résultat : partie créée en mode humain vs humain (`status='waiting'`), puis `/move` → 400.

→ Règle : le champ API est `opponent`, pas `ai_difficulty`. `ai_difficulty` n'existe qu'en DB.

### [APP-E2E-11] Format réponse GET /chess/{id}/moves — Session 32

L'endpoint retourne `Vec<String>` brut en JSON (ex: `["e4","e3","e2"]`), pas un objet.
Ne pas tester `body.moves` ou `body.success` — tester `Array.isArray(body)` et `body.includes('e4')`.

### [APP-E2E-12] Format réponse POST /polls/{id}/vote — Session 32

L'endpoint retourne `{ success: true, poll: PollDetail }`.
Le GET `/api/polls/{id}` retourne `{ poll: PollDetail }`.
Pour accéder à `my_vote` : `detail.poll.my_vote` (pas `detail.my_vote`).
