...
> Généré par `test-nook.yml` · **2026-04-11 19:19 UTC**
...
| **Tests ignorés** | 1 |
| **Total** | 163 |
| **Durée totale** | 1m 39.7s |
| **Branche** | `develop` |
| **Commit** | [`a2e9c21`](https://github.com/MX10-AC2N/Nook/commit/a2e9c218b5b7da348041c81d4b769b652e5390a8) |
| **Run CI** | [Voir le run complet](https://github.com/MX10-AC2N/Nook/actions/runs/24289384035) |
...
| 6 | Créer partie → jouer e2→e4 → IA répond | 3.4s | `?` |
| 7 | Chess — POST /chess/{id}/ai-move → 200 | 2.6s | `?` |
...
| ✅ | Chess — POST /chess/{id}/ai-move → 200 | 2.6s  +1 | |
...
Received: undefined
  ...
  864 |     const game_id = createData.game_id || createData.id || createData.game?.id;
  865 |     console.log('Extracted game_id:', game_id);
> 866 |     expect(game_id).toBeTruthy();
      |                     ^
  867 |     
  868 |     // Naviguer vers la partie
  869 |     await page.goto(`/chess/${game_id}`);
    at /home/runner/work/Nook/Nook/frontend/tests/user.spec.ts:866:21
```
...
WARN nook_backend::auth: Tentative d'accès admin refusée (non-admin) user_id=987b354c-8114-4872-acf2-f3fb13797d26 username=e2e_ci
WARN nook_backend::auth: Tentative d'accès admin refusée (non-admin) user_id=987b354c-8114-4872-acf2-f3fb13797d26 username=e2e_ci
WARN nook_backend::auth: Tentative d'accès admin refusée (non-admin) user_id=987b354c-8114-4872-acf2-f3fb13797d26 username=e2e_ci
```
...
> - **URL du run :** [https://github.com/MX10-AC2N/Nook/actions/runs/24289384035](https://github.com/MX10-AC2N/Nook/actions/runs/24289384035)
> - **Chemin local (CI) :** `frontend/playwright-report/`

Pour examiner visuellement les échecs :
1. Télécharger l'artifact `playwright-report` depuis le [run CI](https://github.com/MX10-AC2N/Nook/actions/runs/24289384035)
...
*Rapport généré par `scripts/generate-test-report.py` — 2026-04-11 19:19 UTC*
