...
> Généré par `test-nook.yml` · **2026-04-11 17:50 UTC**
...
| **Tests ignorés** | 1 |
| **Total** | 163 |
| **Durée totale** | 1m 52.1s |
| **Branche** | `develop` |
| **Commit** | [`2433294`](https://github.com/MX10-AC2N/Nook/commit/24332945c948d82199f631cb031e031b459defc7) |
| **Run CI** | [Voir le run complet](https://github.com/MX10-AC2N/Nook/actions/runs/24287789833) |
...
|------|------|-------|---------|
| 1 | Réactions UI — hover → picker → pill visible | 29.5s | `?` |
| 2 | Chess — sélection pièce → coups légaux visibles (dots) | 12.4s | `?` |
| 3 | Login invalide → reste sur /login | 7.1s | `?` |
| 4 | Chess UI — plateau 64 cases + sélection case + coup via UI | 5.1s | `?` |
| 5 | Register + Approve + Login → accès complet | 4.1s | `?` |
| 6 | /call/default_global → page contient contenu call | 4.0s | `?` |
| 7 | Créer partie → jouer e2→e4 → IA répond | 3.6s | `?` |
| 8 | Chess — POST /chess/{id}/ai-move → 200 | 2.5s | `?` |
| 9 | Avatar — composant visible avec initiales dans le chat | 2.2s | `?` |
| 10 | Admin — approve + login after approve → accès complet | 2.2s | `?` |
...
|--------|------|-------|---------|
| ❌ | Chess — sélection pièce → coups légaux visibles (dots) | 12.4s  +1 | |
| ⚠️ | Réactions UI — hover → picker → pill visible | 29.5s  +1 | |
...
| ✅ | Chess — POST /chess/{id}/ai-move → 200 | 2.5s  +1 | |
...
| ✅ | /call/[id] session → page appel chargee (sans auth first) | 0.4s | |
...
Call log:
  ...
  864 |     // Vérifier que le plateau est visible
  865 |     const board = page.locator('.chess-board');
> 866 |     await expect(board).toBeVisible({ timeout: 10000 });
      |                         ^
  867 |     
  868 |     // Cliquer sur le pion e2
  869 |     const cells = page.locator('.cell');
    at /home/runner/work/Nook/Nook/frontend/tests/user.spec.ts:866:25
```
...
WARN nook_backend::auth: Tentative d'accès admin refusée (non-admin) user_id=35439c10-2537-4bce-b6e7-2ffaeb86f550 username=e2e_ci
WARN nook_backend::auth: Tentative d'accès admin refusée (non-admin) user_id=35439c10-2537-4bce-b6e7-2ffaeb86f550 username=e2e_ci
WARN nook_backend::auth: Tentative d'accès admin refusée (non-admin) user_id=35439c10-2537-4bce-b6e7-2ffaeb86f550 username=e2e_ci
```
...
> - **URL du run :** [https://github.com/MX10-AC2N/Nook/actions/runs/24287789833](https://github.com/MX10-AC2N/Nook/actions/runs/24287789833)
> - **Chemin local (CI) :** `frontend/playwright-report/`

Pour examiner visuellement les échecs :
1. Télécharger l'artifact `playwright-report` depuis le [run CI](https://github.com/MX10-AC2N/Nook/actions/runs/24287789833)
...
*Rapport généré par `scripts/generate-test-report.py` — 2026-04-11 17:50 UTC*
