...
> Généré par `test-nook.yml` · **2026-04-04 14:02 UTC**
...
| **Branche** | `develop` |
| **Commit** | [`e495496`](https://github.com/MX10-AC2N/Nook/commit/e495496c94954244a741b9e07b4dd76edae06973) |
| **Run CI** | [Voir le run complet](https://github.com/MX10-AC2N/Nook/actions/runs/23980293221) |
...
| 5 | Page /admin → non accessible pour user normal | 2.1s | `?` |
| 6 | Chess — POST /chess/{id}/ai-move → 200 | 1.9s | `?` |
| 7 | /call/default_global → page charge avec titres | 1.6s | `?` |
| 8 | Créer partie vs IA (facile) → game_id | 1.1s | `?` |
| 9 | Réactions UI — hover → picker → pill visible | 1.1s | `?` |
...
### ❌ **admin.spec.ts** — 22/25 passés · ❌ 3 failed

| Statut | Test | Durée | Retries |
|--------|------|-------|---------|
| ❌ | Admin — approve + login after approve → accès complet | 0.0s  +1 | |
| ❌ | Admin — DELETE /users/{id} → supprime un utilisateur | 0.0s  +1 | |
| ❌ | GET /analytics → contient user_count, message_count | 0.0s  +1 | |
...
| ✅ | Chess — POST /chess/{id}/ai-move → 200 | 1.9s | |
...
| ✅ | Navigation /events → accessible sans erreur | 0.6s | |
...
ReferenceError: adminPage is not defined
  ...
  331 |   test('Admin — DELETE /users/{id} → supprime un utilisateur', async () => {
  332 |     // Register via admin's authenticated context
> 333 |     const regRes = await adminPage.request.post(`${BASE}/auth/register`, {
      |                    ^
  334 |       data: { username: 'del_t3', password: 'DelTest123!', email: 'dt3@nook.local', name: 'Dt3' },
  335 |     });
  336 |     expect([200, 201, 409]).toContain(regRes.status());
    at /home/runner/work/Nook/Nook/frontend/tests/admin.spec.ts:333:20
```
...
ReferenceError: adminPage is not defined
  ...
  361 |   test('Admin — approve + login after approve → accès complet', async ({ browser }) => {
  362 |     // Create user via admin's authenticated request
> 363 |     const regRes = await adminPage.request.post(`${BASE}/auth/register`, {
      |                    ^
  364 |       data: { username: 'approve_t3', password: 'Approve123!', email: 'apt3@nook.local', name: 'Apt3' },
  365 |     });
  366 |     expect([200, 201, 409]).toContain(regRes.status());
    at /home/runner/work/Nook/Nook/frontend/tests/admin.spec.ts:363:20
```
...
ReferenceError: adminPage is not defined
  ...
  466 |   test('GET /analytics → contient user_count, message_count', async () => {
  467 |     // adminPage already has auth from loginAsAdmin in beforeAll
> 468 |     const res = await adminPage.request.get(`${BASE}/analytics`);
      |                 ^
  469 |     expect(res.status()).toBe(200);
  470 |     const body = await res.json();
  471 |     expect(body).toHaveProperty('user_count');
    at /home/runner/work/Nook/Nook/frontend/tests/admin.spec.ts:468:17
```
...
WARN nook_backend::auth: Tentative d'accès admin refusée (non-admin) user_id=8860a9fa-b380-4c83-a074-edabfe6e9665 username=e2e_ci
WARN nook_backend::auth: Tentative d'accès admin refusée (non-admin) user_id=8860a9fa-b380-4c83-a074-edabfe6e9665 username=e2e_ci
WARN nook_backend::auth: Tentative d'accès admin refusée (non-admin) user_id=8860a9fa-b380-4c83-a074-edabfe6e9665 username=e2e_ci
WARN nook_backend::auth: Tentative d'accès admin refusée (non-admin) user_id=8860a9fa-b380-4c83-a074-edabfe6e9665 username=e2e_ci
```
...
> - **URL du run :** [https://github.com/MX10-AC2N/Nook/actions/runs/23980293221](https://github.com/MX10-AC2N/Nook/actions/runs/23980293221)
> - **Chemin local (CI) :** `frontend/playwright-report/`

Pour examiner visuellement les échecs :
1. Télécharger l'artifact `playwright-report` depuis le [run CI](https://github.com/MX10-AC2N/Nook/actions/runs/23980293221)
...
*Rapport généré par `scripts/generate-test-report.py` — 2026-04-04 14:02 UTC*
