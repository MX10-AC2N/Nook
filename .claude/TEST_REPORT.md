...
> Généré par `test-nook.yml` · **2026-04-10 07:37 UTC**
...
| **Tests ignorés** | 41 |
| **Total** | 159 |
| **Durée totale** | 2m 3.0s |
| **Branche** | `develop` |
| **Commit** | [`78d4bfd`](https://github.com/MX10-AC2N/Nook/commit/78d4bfda36033b7a7c46674d9059cd4931e57a8f) |
| **Run CI** | [Voir le run complet](https://github.com/MX10-AC2N/Nook/actions/runs/24231428156) |
...
|------|------|-------|---------|
| 1 | Chat UI — sidebar et envoi message | 1m 20.3s | `?` |
...
| 5 | Créer partie → jouer e2→e4 → IA répond | 3.0s | `?` |
...
| 10 | DELETE /users/{id} → supprime un utilisateur | 1.1s | `?` |
...
|--------|------|-------|---------|
| ❌ | Chat UI — sidebar et envoi message | 1m 20.3s  +1 | |
...
Locator: locator('.message-content').filter({ hasText: 'E2E message 1775806567836' })
Expected: visible
Timeout: 15000ms
...
Call log:
  ...
  - waiting for locator('.message-content').filter({ hasText: 'E2E message 1775806567836' })


  126 |
  127 |     // Check message appears in DOM
> 128 |     await expect(page.locator('.message-content').filter({ hasText: msgText })).toBeVisible({ timeout: 15_000 });
      |                                                                                 ^
  129 |     console.log('✅ Message envoyé et affiché dans le DOM');
  130 |   });
  131 |
    at /home/runner/work/Nook/Nook/frontend/tests/user.spec.ts:128:81
```
...
Locator: locator('.message-content').filter({ hasText: 'E2E message 1775806613823' })
Expected: visible
Timeout: 15000ms
Error: element(s) not found
...
Call log:
  - Expect "toBeVisible" with timeout 15000ms
  - waiting for locator('.message-content').filter({ hasText: 'E2E message 1775806613823' })
...
WARN nook_backend::auth: Tentative d'accès admin refusée (non-admin) user_id=3834635e-61e8-4553-bcb0-a44a40edae0e username=e2e_ci
WARN nook_backend::auth: Tentative d'accès admin refusée (non-admin) user_id=3834635e-61e8-4553-bcb0-a44a40edae0e username=e2e_ci
```
...
> - **URL du run :** [https://github.com/MX10-AC2N/Nook/actions/runs/24231428156](https://github.com/MX10-AC2N/Nook/actions/runs/24231428156)
> - **Chemin local (CI) :** `frontend/playwright-report/`

Pour examiner visuellement les échecs :
1. Télécharger l'artifact `playwright-report` depuis le [run CI](https://github.com/MX10-AC2N/Nook/actions/runs/24231428156)
...
*Rapport généré par `scripts/generate-test-report.py` — 2026-04-10 07:37 UTC*
