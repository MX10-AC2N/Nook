...
> Généré par `test-nook.yml` · **2026-04-12 08:00 UTC**
...
| **Tests ignorés** | 35 |
| **Total** | 163 |
| **Durée totale** | 1m 22.5s |
| **Branche** | `develop` |
| **Commit** | [`666ac2d`](https://github.com/MX10-AC2N/Nook/commit/666ac2dd8e37a3bf447465eefc78c484c593e66c) |
| **Run CI** | [Voir le run complet](https://github.com/MX10-AC2N/Nook/actions/runs/24301701143) |
...
|------|------|-------|---------|
| 1 | Réactions UI — hover → picker → pill visible | 36.8s | `?` |
...
| 5 | Créer partie → jouer e2→e4 → IA répond | 3.7s | `?` |
...
| 10 | /call/fake-id avec auth → page charge | 1.2s | `?` |
...
|--------|------|-------|---------|
| ❌ | Réactions UI — hover → picker → pill visible | 36.8s  +1 | |
...
Call log:
  ...
  282 |     const msg = page.locator('.message').last();
> 283 |     await expect(msg).toBeVisible({ timeout: 20_000 });
      |                       ^
  284 |
  285 |     // Click sur le message pour déclencher les actions (plus fiable que hover en CI)
  286 |     await msg.click();
    at /home/runner/work/Nook/Nook/frontend/tests/user.spec.ts:283:23
```
...
Locator: locator('[data-testid="reaction-trigger"]').last()
Expected: visible
Timeout: 10000ms
Error: element(s) not found
...
Call log:
  ...
  - waiting for locator('[data-testid="reaction-trigger"]').last()


  289 |     // Cliquer sur le bouton réaction via data-testid
  290 |     const reactionTrigger = page.locator('[data-testid="reaction-trigger"]').last();
> 291 |     await expect(reactionTrigger).toBeVisible({ timeout: 10_000 });
      |                                   ^
  292 |     await reactionTrigger.click();
  293 |
  294 |     // Picker visible via data-testid
    at /home/runner/work/Nook/Nook/frontend/tests/user.spec.ts:291:35
```
...
WARN nook_backend::auth: Tentative d'accès admin refusée (non-admin) user_id=4e1a787b-1665-414f-bc4d-f2ce99f62c7d username=e2e_ci
WARN nook_backend::auth: Tentative d'accès admin refusée (non-admin) user_id=4e1a787b-1665-414f-bc4d-f2ce99f62c7d username=e2e_ci
```
...
> - **URL du run :** [https://github.com/MX10-AC2N/Nook/actions/runs/24301701143](https://github.com/MX10-AC2N/Nook/actions/runs/24301701143)
> - **Chemin local (CI) :** `frontend/playwright-report/`

Pour examiner visuellement les échecs :
1. Télécharger l'artifact `playwright-report` depuis le [run CI](https://github.com/MX10-AC2N/Nook/actions/runs/24301701143)
...
*Rapport généré par `scripts/generate-test-report.py` — 2026-04-12 08:00 UTC*
