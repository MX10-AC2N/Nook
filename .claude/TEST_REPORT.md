...
> Généré par `test-nook.yml` · **2026-04-11 18:33 UTC**
...
| **Tests ignorés** | 1 |
| **Total** | 163 |
| **Durée totale** | 1m 38.4s |
| **Branche** | `develop` |
| **Commit** | [`b394a55`](https://github.com/MX10-AC2N/Nook/commit/b394a5549ca8804437d17e1001684157c08ccd51) |
| **Run CI** | [Voir le run complet](https://github.com/MX10-AC2N/Nook/actions/runs/24288579227) |
...
| 4 | Register + Approve + Login → accès complet | 4.2s | `?` |
| 5 | /call/default_global → page contient contenu call | 4.0s | `?` |
| 6 | Créer partie → jouer e2→e4 → IA répond | 3.3s | `?` |
| 7 | Avatar — composant visible avec initiales dans le chat | 2.2s | `?` |
| 8 | Admin — approve + login after approve → accès complet | 2.2s | `?` |
| 9 | Page /admin → non accessible pour user normal | 2.1s | `?` |
| 10 | Chess — POST /chess/{id}/ai-move → 200 | 2.1s | `?` |
...
Received: undefined
  ...
  860 |     expect(createRes.status()).toBeLessThan(500);
  861 |     const { game_id } = await createRes.json();
> 862 |     expect(game_id).toBeTruthy();
      |                     ^
  863 |     
  864 |     // Naviguer vers la partie
  865 |     await page.goto(`/chess/${game_id}`);
    at /home/runner/work/Nook/Nook/frontend/tests/user.spec.ts:862:21
```
...
WARN nook_backend::auth: Tentative d'accès admin refusée (non-admin) user_id=b9b6d57b-8370-427c-a7a0-09914ac5b1d9 username=e2e_ci
WARN nook_backend::auth: Tentative d'accès admin refusée (non-admin) user_id=b9b6d57b-8370-427c-a7a0-09914ac5b1d9 username=e2e_ci
WARN nook_backend::auth: Tentative d'accès admin refusée (non-admin) user_id=b9b6d57b-8370-427c-a7a0-09914ac5b1d9 username=e2e_ci
```
...
> - **URL du run :** [https://github.com/MX10-AC2N/Nook/actions/runs/24288579227](https://github.com/MX10-AC2N/Nook/actions/runs/24288579227)
> - **Chemin local (CI) :** `frontend/playwright-report/`

Pour examiner visuellement les échecs :
1. Télécharger l'artifact `playwright-report` depuis le [run CI](https://github.com/MX10-AC2N/Nook/actions/runs/24288579227)
...
*Rapport généré par `scripts/generate-test-report.py` — 2026-04-11 18:33 UTC*
