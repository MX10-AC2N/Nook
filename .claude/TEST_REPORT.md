...
> Généré par `test-nook.yml` · **2026-04-11 20:46 UTC**
...
| **Tests ignorés** | 1 |
| **Total** | 163 |
| **Durée totale** | 1m 38.8s |
| **Branche** | `develop` |
| **Commit** | [`b439432`](https://github.com/MX10-AC2N/Nook/commit/b439432589a67d88912b003eb1b8946c3d335b3e) |
| **Run CI** | [Voir le run complet](https://github.com/MX10-AC2N/Nook/actions/runs/24290961386) |
...
| 6 | Créer partie → jouer e2→e4 → IA répond | 3.6s | `?` |
| 7 | Chess — POST /chess/{id}/ai-move → 200 | 2.5s | `?` |
| 8 | Avatar — composant visible avec initiales dans le chat | 2.3s | `?` |
| 9 | Admin — approve + login after approve → accès complet | 2.2s | `?` |
| 10 | Page /admin → non accessible pour user normal | 2.2s | `?` |
...
| ✅ | Avatar — composant visible avec initiales dans le chat | 2.3s  +1 | |
| ✅ | /call/default_global → page charge avec titres | 1.6s | |
| ✅ | Settings — section avatar visible avec grille d'options | 1.2s  +1 | |
...
| ✅ | Upload — fichier texte → file_id, url=/api/download/, download OK | 0.1s  +1 | |
...
**Message :**
```
Error: Chess create failed. Status: status() {
    return this._initializer.status;
  }, Response: {"message":"Non authentifié","success":false}

  861 |     // Debug: show actual response
  862 |     if (!createData.game_id && !createData.id) {
> 863 |       throw new Error('Chess create failed. Status: ' + createRes.status + ', Response: ' + JSON.stringify(createData));
      |             ^
  864 |     }
  865 |     expect(createRes.status()).toBeLessThan(500);
  866 |     const game_id = createData.game_id || createData.id;
    at /home/runner/work/Nook/Nook/frontend/tests/user.spec.ts:863:13
```
...
WARN nook_backend::auth: Tentative d'accès admin refusée (non-admin) user_id=532decc1-4ecf-4b10-8da4-92c0227c500b username=e2e_ci
WARN nook_backend::auth: Tentative d'accès admin refusée (non-admin) user_id=532decc1-4ecf-4b10-8da4-92c0227c500b username=e2e_ci
WARN nook_backend::auth: Tentative d'accès admin refusée (non-admin) user_id=532decc1-4ecf-4b10-8da4-92c0227c500b username=e2e_ci
```
...
> - **URL du run :** [https://github.com/MX10-AC2N/Nook/actions/runs/24290961386](https://github.com/MX10-AC2N/Nook/actions/runs/24290961386)
> - **Chemin local (CI) :** `frontend/playwright-report/`

Pour examiner visuellement les échecs :
1. Télécharger l'artifact `playwright-report` depuis le [run CI](https://github.com/MX10-AC2N/Nook/actions/runs/24290961386)
...
*Rapport généré par `scripts/generate-test-report.py` — 2026-04-11 20:46 UTC*
