...
> Généré par `test-nook.yml` · **2026-04-10 06:58 UTC**
...
| **Tests ignorés** | 41 |
| **Total** | 159 |
| **Durée totale** | 1m 15.7s |
| **Branche** | `develop` |
| **Commit** | [`1b4d286`](https://github.com/MX10-AC2N/Nook/commit/1b4d28685910a5a9be5ad73350628798375c4e58) |
| **Run CI** | [Voir le run complet](https://github.com/MX10-AC2N/Nook/actions/runs/24230139440) |
...
|------|------|-------|---------|
| 1 | Chat UI — sidebar et envoi message | 30.3s | `?` |
| 2 | Login invalide → reste sur /login | 7.1s | `?` |
| 3 | Register + Approve + Login → accès complet | 4.3s | `?` |
| 4 | /call/default_global → page contient contenu call | 4.2s | `?` |
| 5 | Créer partie → jouer e2→e4 → IA répond | 3.3s | `?` |
| 6 | Admin — approve + login after approve → accès complet | 2.2s | `?` |
| 7 | Page /admin → non accessible pour user normal | 2.2s | `?` |
| 8 | Admin — DELETE /users/{id} → supprime un utilisateur | 2.1s | `?` |
| 9 | /call/default_global → page charge avec titres | 1.7s | `?` |
...
|--------|------|-------|---------|
| ❌ | Chat UI — sidebar et envoi message | 30.3s  +1 | |
| ✅ | Login invalide → reste sur /login | 7.1s  +1 | |
| ✅ | /call/default_global → page contient contenu call | 4.2s | |
...
| ✅ | Page /admin → non accessible pour user normal | 2.2s | |
...
| ✅ | Flux inscription : register → pending → approve → connecté | 0.2s | |
...
Locator: locator('.message-content').filter({ hasText: 'E2E message 1775804256214' })
Expected: visible
Timeout: 15000ms
...
Call log:
  ...
  - waiting for locator('.message-content').filter({ hasText: 'E2E message 1775804256214' })
...
Locator: locator('.message-content').filter({ hasText: 'E2E message 1775804277451' })
Expected: visible
Timeout: 15000ms
Error: element(s) not found
...
Call log:
  - Expect "toBeVisible" with timeout 15000ms
  - waiting for locator('.message-content').filter({ hasText: 'E2E message 1775804277451' })
...
WARN nook_backend::auth: Tentative d'accès admin refusée (non-admin) user_id=fac8ce66-5669-4fbe-9971-8d5c64b56394 username=e2e_ci
WARN nook_backend::auth: Tentative d'accès admin refusée (non-admin) user_id=fac8ce66-5669-4fbe-9971-8d5c64b56394 username=e2e_ci
```
...
> - **URL du run :** [https://github.com/MX10-AC2N/Nook/actions/runs/24230139440](https://github.com/MX10-AC2N/Nook/actions/runs/24230139440)
> - **Chemin local (CI) :** `frontend/playwright-report/`

Pour examiner visuellement les échecs :
1. Télécharger l'artifact `playwright-report` depuis le [run CI](https://github.com/MX10-AC2N/Nook/actions/runs/24230139440)
...
*Rapport généré par `scripts/generate-test-report.py` — 2026-04-10 06:58 UTC*
