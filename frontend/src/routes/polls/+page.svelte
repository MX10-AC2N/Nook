<script lang="ts">
  let polls = [
    { question: 'Pizza ou sushi ?', options: ['Pizza', 'Sushi'], votes: [3, 5] }
  ];
...
<div class="p-4">
  ...
  <div class="mb-6">
    ...
    <button on:click={createPoll} class="bg-purple-500 text-white p-2 rounded">Créer un sondage</button>
  </div>
    ...
    <div class="mb-4 p-4 border rounded">
      ...
      {#each poll.options as option, j}
        <div class="flex items-center mb-1">
          <button on:click={() => vote(i, j)} class="mr-2 bg-gray-200 px-2 py-1 rounded">🗳️</button>
